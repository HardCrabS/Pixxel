...
namespace Mopsicus.InfiniteScroll
{
    ...
    public class InfiniteScroll : MonoBehaviour, IDropHandler
    {
        ...
        public void InitData(int count, int startIndex = 0)
        {
            if (Type == 0)
            {
                InitVertical(count, startIndex);
            }
            else
            {
        ...
        void InitVertical(int count, int startIndex = 0)
        {
            float height = CalcSizesPositions(count);
            CreateViews();
            _previousPosition = startIndex == 0 ? 0 : Mathf.Clamp(startIndex + 1, startIndex, count - 1);
            startIndex = startIndex == 0 ? 0 : Mathf.Clamp(startIndex - 2, 0, count - 1);
            _count = count;
            _content.sizeDelta = new Vector2(_content.sizeDelta.x, height);
            Vector2 pos = _content.anchoredPosition;
            Vector2 size = Vector2.zero;
            pos.y = 0f;
            pos.y = GetHeight(startIndex);
            ...
            for (int i = 0; i < _views.Length; i++)
            {
                ...
                if (startIndex + i + 1 > _count)
                {
                    continue;
                }
                pos = _rects[i].anchoredPosition;
                pos.y = _positions[startIndex + i];
                pos.x = 0f;
                _rects[i].anchoredPosition = pos;
                size = _rects[i].sizeDelta;
                size.y = _heights[i];
                _rects[i].sizeDelta = size;
                y += ItemSpacing + _heights[startIndex + i];
                _views[i].name = (startIndex + i).ToString();
                OnFill(startIndex + i, _views[i]);
            }
        }
        ...
        public IEnumerator MoveByDelta(Direction direction, int playerIndex)
        {
            float speed = SCROLL_SPEED;
            float start = 0f;
            float end = 0f;
            ...
            playerIndex++;
            float height = GetHeight(_count);
            float playerHeight = playerIndex * _heights[0] + (playerIndex - 1) * ItemSpacing + TopPadding;
            float delta = playerHeight / height;
            delta *= direction == Direction.Top ? 1 : -1;

            start = _scroll.verticalNormalizedPosition;
            end = _scroll.verticalNormalizedPosition + delta;
            while (timer <= 1f)
            {
                //speed = Mathf.Lerp(speed, 0f, timer);
